...
function QiniuJsSDK() {
    ...
    this.parseJSON = function(data) {
        ...
        if (window.JSON && window.JSON.parse) {
        	if(data.indexOf("{\"saveKey\":")!=-1||data.indexOf("{\"scope\":")!=-1)
        	{
        		var lengthStr="deadline\":";
        		data= data.substr(0,data.indexOf("}")+1);
        		//data=data.substr(0,data.indexOf(lengthStr)+lengthStr.length)+"\""+timest111()+"\"}";
        	}
    ...
    this.uploader = function(op) {
        ...
        var getNewUpToken = function(file) {
            if (op.uptoken) {
                ...
                if (ajax.status === 200) {
                    //var res = that.parseJSON(ajax.responseText);
                    that.token =ajax.responseText.replace("\"","").replace("\"","");// res.uptoken;
                   // that.token="xV3C6VmnwuN_12h12YFLJOJ-Y9SyYUIHmpZIk0jb:FC4eMfPHczOAm1UkNsl-DbfeZck=:eyJzY29wZSI6ImxpYnVidWNrZXQiLCJkZWFkbGluZSI6MTQ4ODEwNjY0Nn0=";
        ...
        var getFileKey = function(up, file, func) {
            ...
            if (!op.save_key) {
                ...
                if (unique_names) {
                    ...
                    var tmp = (Date.parse( new Date() )/1000).toString()+randomNum;
