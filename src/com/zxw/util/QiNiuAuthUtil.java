...
public class QiNiuAuthUtil {
    ...
    private QiNiuAuthUtil(String accessKey, SecretKeySpec secretKeySpec) {
        this.accessKey = accessKey;
        this.secretKey = secretKeySpec;
    }

    public static QiNiuAuthUtil create(String accessKey, String secretKey) {
        if (StringUtils.isNullOrEmpty(accessKey) || StringUtils.isNullOrEmpty(secretKey)) {
            throw new IllegalArgumentException("empty key");
        }
        byte[] sk = StringUtils.utf8Bytes(secretKey);
        SecretKeySpec secretKeySpec = new SecretKeySpec(sk, "HmacSHA1");
        return new QiNiuAuthUtil(accessKey, secretKeySpec);
    }
    ...
    public String uploadToken(String bucket, String key, long expires, StringMap policy, boolean strict) {
        long deadline = System.currentTimeMillis() / 1000 + expires;  
//        try { 
//			deadline= new java.text.SimpleDateFormat("dd/MM/yyyy HH:mm:ss").
//			parse("02/26/2018 17:53:00").getTime()/1000;
...
//		}
    ...
    public String uploadTokenWithPolicy(StringMap obj) {
    ...
    try {
		long	deadline = new java.text.SimpleDateFormat("dd/MM/yyyy HH:mm:ss").
					parse("02/26/2018 17:53:00").getTime();
